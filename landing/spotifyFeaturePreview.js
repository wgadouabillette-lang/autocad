(function () {
  ...
  var FEATURE_MESSAGE = "lyte-marketing-spotify-feature";

  var ZOOM = {
    hold: { vw: 700, vh: 500 },
    composer: { vw: 600, vh: 430 },
    results: { vw: 640, vh: 560 },
    playing: { vw: 700, vh: 500 },
  };

  var BIAS = {
    hold: { x: 0.7, y: 0.76 },
    composer: { x: 0.7, y: 0.76 },
    results: { x: 0.66, y: 0.22 },
    playing: { x: 0.7, y: 0.76 },
  };
  ...
  function resolvePreviewHref() {
    ...
    params.set("scene", "spotify");
    return "/app/preview.html?" + params.toString();
  }

  function showFallback(mount) {
    mount.innerHTML =
      '<img class="hero__feature-img" src="app-preview.png" alt="Meetra Spotify preview" loading="eager" decoding="async" />';
  }
  ...
  function setSpotifyCopyWash(on) {
    var copy = document.querySelector(".hero__feature-copy--spotify");
    if (!copy) return;
    copy.classList.toggle("is-spotify-playing", !!on);
  }
  ...
  function mountPreview() {
    var mount = document.getElementById("spotify-feature-preview");
    if (!mount) return;
    ...
    iframe.title = "Meetra Spotify preview";
